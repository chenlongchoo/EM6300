...
void ModbusTCPServerTask(void)
{
	...
	static BYTE bExpected_Request_DataSize;
	...
	for(vModbusSession = 0; vModbusSession < MAX_MODBUS_CONNECTIONS; vModbusSession++)
	{
		...
		switch(ModbusServerState)
		{
			case MBSS_HOME: // Opens the socket at the specified port to listen for request.
			{
				...
				bExpected_Request_DataSize = sizeof(MODBUS_REQUEST_BLOCK);
			}	
				break;
			...
			case MBSS_LISTENING:  // This listens out for new requests and checks the message header.
			{
				 ...
				 */
				{
					// No point putting TCPIsGetReady() in a while loop. We need to process whatever is available 
					// and exit since more data will only be available after the next loop.
					w = TCPIsGetReady(MySocket);
					//BYTE bExpected_Request_DataSize = sizeof(MODBUS_REQUEST_BLOCK);
					if( w > bExpected_Request_DataSize )
						w = bExpected_Request_DataSize;
					...
					bExpected_Request_DataSize -= received;
					if( received == sizeof(MODBUS_REQUEST_BLOCK) )
					{
						// 2012-05-08(Eric) - We must reset this variable in preparation to receive new request.
						received = 0;
