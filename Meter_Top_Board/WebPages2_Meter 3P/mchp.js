	...
	for(i = ajaxList.length; i > 0; i--)
	{
		...
		if(curAjax.ajaxReq.readyState == 4 && curAjax.ajaxReq.status == 200) {
			...
			if(typeof(curAjax.container) == 'function'){
				if( !curAjax.ajaxReq.responseXML ) curAjax.container(curAjax.ajaxReq.responseText);
				else curAjax.container(curAjax.ajaxReq.responseXML.documentElement);
			} else if(typeof(curAjax.container) == 'string') {
				/* 2012-04-25(Eric) - Customised to allow return value to be written into input elements. */
				e = document.getElementById(curAjax.container);
				if(e!=null && e.nodeName!=null && e.nodeName=="INPUT")
				    e.value = curAjax.ajaxReq.responseText;
				else 
		...
		if(elapsed > timeOutMS) {
			...
			if(typeof(curAjax.container) == 'function'){
				...
				//ajaxList = new Array();
				// Alert the user
				//alert("Command failed.\nConnection to development board was lost. " + curAjax.url);
			}
